    ...
    if (waypoints && waypoints.length > 0) {
      waypoints.forEach((waypoint, index) => {
        const numberedIcon = L.divIcon({
          className: '',
          html: `<div style="background:linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%);color:white;border-radius:50%;width:28px;height:28px;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:bold;border:2px solid white;box-shadow:0 2px 8px rgba(99, 102, 241, 0.5);">${index + 1}</div>`,
          iconSize: [28, 28],
          iconAnchor: [14, 14],
        });
